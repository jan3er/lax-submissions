...
variable {g : List ℕ} {n m : ℕ} {G : SimpleGraph (Fin n)} {O T MK : ℕ → ℕ}
...
slot. Everything but its six working scalars is untouched. -/
theorem scan_run (hg : EncodesGraph g n G) (hm : edgeCount g = m)
  ...
  have hstep : ∀ τ, ScanInv g n m MK σ τ →
      (Cond.lt (.var "j") (.var "m2")).eval τ = some true →
      ∃ τ' K, Run scanBody τ τ' K ∧ ScanInv g n m MK σ τ' ∧
        1 + (Cond.lt (Expr.var "j") (Expr.var "m2")).size + K + ScanPot m n τ' ≤
          ScanPot m n τ := by
    rintro τ ⟨hm2', harrs, hinp', hout', hfr, hun, houj, hj2m, hdich⟩ hcond
    have hjlt : τ.vars "j" < 2 * m := by simpa [hm2'] using hcond
    ...
    · -- `j` is inside the owner's block: examine the slot
      have hc1 : (Cond.lt (.var "j")
          (.get "off" (.add (.var "u") (.lit 1)))).eval τ = some true := by
      ...
      have r_w : Run (.assign "w" (.get "tgt" (.var "j"))) τ
          (τ.setVar "w" (T (τ.vars "j"))) 3 :=
        (Run.assign (by simp [harrs, htgt, getElem?_arrOf T hjlt])).mono (by simp)
      by_cases hMKu : MK (τ.vars "u") = 0
      · have hc2 : (Cond.eq (.get "mark" (.var "u")) (.lit 0)).eval
        ...
        · -- an uncovered edge: record it and force the exit
          set τ' : Env := ((((τ.setVar "w" (T (τ.vars "j"))).setVar "eu"
              ...
              "j" (2 * m) with hτ'
          have hc3 : (Cond.eq (.get "mark" (.var "w")) (.lit 0)).eval
              (τ.setVar "w" (T (τ.vars "j"))) = some true := by
            simp [harrs, hmark, hTj, getElem?_arrOf MK hwlt, hMKw]
          refine ⟨τ', _, Run.ite_true hc1 (Run.seq r_w (Run.ite_true hc2
            (Run.ite_true hc3 (Run.seq (Run.assign (v := τ.vars "u") (by simp))
              (Run.seq (Run.assign (v := T (τ.vars "j")) (by simp))
                (Run.seq (Run.assign (v := 1) (by simp))
                  (Run.assign (v := 2 * m) (by simp [hm2'])))))))), ?_, ?_⟩
        ...
        · -- the target is marked: the slot is covered, advance `j`
          ...
          have hc3 : (Cond.eq (.get "mark" (.var "w")) (.lit 0)).eval
            ...
            exact hMKw
          refine ⟨τ', _, Run.ite_true hc1 (Run.seq r_w (Run.ite_true hc2
            (Run.ite_false hc3 (Run.assign (v := τ.vars "j" + 1) (by simp))))), ?_, ?_⟩
          · refine ⟨by simp [hτ', hm2'], by simp [hτ', harrs], by simp [hτ', hinp'],
      ...
      · -- the owner is marked: the slot is covered, advance `j`
        ...
        refine ⟨τ', _, Run.ite_true hc1 (Run.seq r_w (Run.ite_false hc2
          (Run.assign (v := τ.vars "j" + 1) (by simp)))), ?_, ?_⟩
        · refine ⟨by simp [hτ', hm2'], by simp [hτ', harrs], by simp [hτ', hinp'],
            by simp [hτ', hout'], ?_, by simp [hτ']; omega,
            by simp [hτ']; omega, by simp [hτ']; omega, ?_⟩
          · intro y h1 h2 h3 h4 h5 h6
    ...
    · -- the owner's block is exhausted: advance the owner
      have hc1 : (Cond.lt (.var "j")
          (.get "off" (.add (.var "u") (.lit 1)))).eval τ = some false := by
        simp [harrs, hoff, getElem?_arrOf O (show τ.vars "u" + 1 < n + 1 by omega),
          hOu1]
        omega
      refine ⟨τ.setVar "u" (τ.vars "u" + 1), _,
        Run.ite_false hc1 (Run.assign (v := τ.vars "u" + 1) (by simp)), ?_, ?_⟩
  ...
  obtain ⟨τ', K, hrun, ⟨hm2', harrs', hinp'', hout'', hfr', hun', houj', hj2m', hdich'⟩,
      hfalse, hpay⟩ :=
    Run.while_pot (b := Cond.lt (.var "j") (.var "m2")) (c := scanBody)
      (ScanInv g n m MK σ) (ScanPot m n) (fun τ _ => ⟨_, rfl⟩) hstep hI₀
  ...
  have hKle : K ≤ 100 * m + 50 * n + 4 := by
    ...
    omega
  refine ⟨τ', ((Run.seq (Run.assign (v := 0) rfl)
    (Run.seq (Run.assign (v := 0) rfl)
      (Run.seq (Run.assign (v := 0) rfl) hrun))).mono (by simp; omega)),
