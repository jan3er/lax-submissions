...
theorem initLab_run {B : ℕ} {σ : Env} {g : ℕ → ℕ} {n : ℕ}
  ...
  have hstep : ∀ τ : Env, InitInv σ n τ →
      ...
      ∃ τ', Run B (.seq (.store "lab" (.var "i") (.var "n"))
...
theorem writeLoop_run {B : ℕ} {σ : Env} {g : ℕ → ℕ} {n : ℕ}
  ...
  have hik : σ'.vars "i" = n := by simp [hnn'] at hfalse; omega
  rw [hik] at hout'
  exact ⟨σ', (Run.seq (Run.assign (v := 0) (by simp; omega)) hrun).mono (by simp; omega), hout'⟩
