...
source twin-width; `Lax2Proofs.Bridge` proves both submitted parameters
pointwise equal to their source counterparts, so the bound transports
verbatim.
-/

namespace Lax2Proofs.MixedMinorNumberFromTwinWidth

noncomputable section

/--
---
conclusion: Lax2.MixedMinorNumberFromTwinWidth.exists_mixedMinorNumber_bound_of_twinWidth
---
The submitted mixed minor number is bounded by a numerical function of
twin-width (the parameter of submission Lax1); the witness is the linear
function `d ↦ 2 * (d + 3) + 2`.
...
`Lax2Proofs.Bridge` moves the bound onto the submitted parameters.
...
theorem exists_mixedMinorNumber_bound_of_twinWidth :
    ∃ g : ℕ → ℕ, ∀ {V : Type} [Fintype V] [DecidableEq V]
      ...
      Lax2.MixedMinorNumber.mixedMinorNumber G ≤
        g (Lax1.TwinWidth.twinWidth G) := by
...
end Lax2Proofs.MixedMinorNumberFromTwinWidth
