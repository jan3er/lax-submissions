...
statement of this submission's own concept `Lax12.NowhereDenseWcol`, rather
...
/--
---
conclusion: Lax12.NowhereDenseNC.hasAlmostLinearNC_of_nowhereDense
assumptions:
...
statement `Lax12.NowhereDenseWcol.hasSubpolynomialWcol_of_nowhereDense`.
...
theorem hasAlmostLinearNC_of_nowhereDense (C : GraphClass)
  ...
  have hδ : 0 < δ := by
    ...
    positivity
  obtain ⟨cw, hcw⟩ :=
    Lax12.NowhereDenseWcol.hasSubpolynomialWcol_of_nowhereDense C h 2 δ hδ
  let cw' : ℝ := max cw 0
  let K : ℕ := 3 * t + 1
  let L : ℕ := 1 + K + K ^ 2
  let M : ℝ := (K + 1 : ℕ) * (2 : ℝ) ^ s
...
end Lax12Proofs.NowhereDenseNC
