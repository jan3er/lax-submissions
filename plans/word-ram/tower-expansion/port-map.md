...
| matrices: `Matrix` intf + array matrix | AFP `IICF_Array_Matrix` (23.5 KB) + Sepreftime (10.4 KB, cost) | P5.A | Floyd–Warshall-class; PCP constraint graphs | **P5.C — `Array_Matrix` green as an unrooted leaf (1/1 matrix family; 3/5 P5.C families)** |
| heaps / prio maps: `Abs_Heap`, `Impl_Heap`, `Abs_Heapmap`, `Impl_Heapmap` | Sepreftime cost copies (33.6 / 6.5 / 50.5 / 32.7 KB); artifact copies dead (F1); `isabelle_llvm` 2023 shapes | P5.A/B | Dijkstra/Prim-class; PCP expanders | **P5.C — `Abs_Heap` and `Impl_Heap` green as unrooted leaves (2/4 heap families; 3/5 P5.C families); `Abs_Heapmap` next** |
