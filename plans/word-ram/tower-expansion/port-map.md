...
| array lists / resizable arrays: `Array_List`, `DArray_List` (dynamic-array-backed), `MS_Array_List`, `Indexed_Array_List`, `Array_of_Array_List` | Sepreftime cost copies (8.8 / 3.1 / 8.5 / 16.9 KB) + artifact `Array_of_Array_List` (9.1, + `ds/` twin 14.5 shape); no-cost twins for shape | P4.A (dyn array), P5.A | bounded sequences; general | **P5.B — first four families green as unrooted leaves (4/5 sequence families; 4/8 P5.B implementation families); `Array_of_Array_List` next** |
