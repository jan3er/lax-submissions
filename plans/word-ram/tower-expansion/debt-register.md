...
| SIG-2 | OPEN P1.A | `to_hnr`/`to_hfref`, `comp_PRE`, and the source's `FCOMP` composition attribute are absent. | `Sepref/Rules.lean` D-o explicitly ports lemma content only. Source: `Sepref_Rules.thy`. | A signature theorem composes through a pure relator without applying `hfcomp` by hand. |
| SIG-3 | OPEN P1.A | Iterated dependent composition has no flattening lemma. | `Sepref/Rules.lean`, “Deliberate absences”; local name planned as `hrrCompDep` flattening. | A loop-shaped two-stage composition normalizes to the postcondition expected by the next rule. |
| SIG-4 | OPEN P1.A | `hr_comp_precise`, `hr_comp_the_pure`, `hr_comp_assoc`, and `hr_comp_prod_conv` were skipped. | `Sepref/Rules.lean`, “Deliberate absences”; source `Sepref_Rules.thy`. | Port the source lemmas or record, per lemma, why P1 acceptance cannot consume it. |
| SIG-5 | OPEN P1.A | `one_time`/`one_time_attains_sup` and the `attains_sup_mop_*` family are absent. | `Sepref/Rules.lean`, “Deliberate absences.” | Source tutorial composition closes without ad-hoc `attainsSup` proofs, or P1 records the exact exclusion. |
| SIG-6 | OPEN P1.B | `sepref_definition`/`sepref_thm` prepares hand-holed `hnRefine` goals only partially. | `Sepref/Definition.lean`; the present command still starts from explicit judgment text. | `BfsQSynth` is regenerated from an `hfref` signature with byte-identical `Com` and no hand-written goal text. |
| SIG-7 | OPEN P1.B | `sepref_register`, `intf_of_assn`, and TYPE/interface annotations are absent. | Source `Sepref_Combinator_Setup.thy` and `Sepref_Id_Op.thy`; port map P1.B. | Operators identify from assertions across the ≥8-array acceptance toy. |
| SIG-8 | OPEN P1.C | `sepref_decl_op`, `sepref_decl_intf`, and `sepref_decl_impl` command layer is absent. | Source `Sepref_Intf_Util.thy`. | P5 can declare one new interface and implementation without bespoke metaprogramming. |
