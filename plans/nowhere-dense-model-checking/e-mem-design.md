...
| dead sweep / base tables | `DeadSweep.sweepImplements`, `RamDriverBot.baseImplements` | NOT header changes — compiled refutation (`DeadSweep.lean` §4b, landed 2026-08-06): the sweep's whole output (`DeadRows`) quantifies over the DEAD set, the exact complement of the member list (`no_memCoeff_pays_deadRows`); the base is blocked twice (`TableInv` over all `Fin n`; `BotEval.sat_exU_bot_of_repr`'s `hW` ranges the bottom quantifier over the carrier). Both go to the R1.8-design leaf: death-row writes move to the killing turn (its block contains the vertex), and the base needs a representative story for dead vertices |
