...
theorem warrs_clusterLoad (j : ℕ) : (clusterLoad j).warrs = [cluName j, cluName j] := rfl
...
theorem underscore_notMem_warrs_descendCom (cap j : ℕ) :
  ...
  rcases ha with (rfl | rfl) | rfl | (rfl | rfl | h) | h | rfl | rfl | rfl
  · exact hclu
  · exact hclu
  · exact hres
  · exact hbal
...
`hinner`'s precondition also asks `RamDriver.PlayOk` of the mask, which
`InnerFrames` does not carry and cannot, since it names no oracle.
...
`inner` is guarded by `RamDriver.PlayOk` at depth `j + 1`, and its
...
theorem clusterFrames {Or : PathOracle n (2 * cap)} {inner : Com}
  ...
  have hframe : σ₅.arrs (tabName j i) = σ.arrs (tabName j i) := by
    rw [hr₅.frame_arr _ hfs, hr₄.frame_arr _ (hinnerTab i), hr₃.frame_arr _ hfc,
      hr₂.frame_arr _ hfe, hr₁.frame_arr _ hfd]
